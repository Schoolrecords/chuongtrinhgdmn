...
<footer class="site-footer" role="contentinfo">
  <div class="container">
    <div>
      <div class="footer-brand"><span class="brand-logo${s.logo ? ' has-img' : ''}" aria-hidden="true">${s.logo ? `<img src="${esc(s.logo)}" alt="" width="40" height="40">` : icon('book-open')}</span><div><b>${esc(s.nameUpper || s.name)}</b><span>${esc(s.siteTitle)} · ${esc(s.schoolYearLabel)}</span></div></div>
